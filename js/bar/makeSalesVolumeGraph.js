...
makeSalesVolumeGraph(salesVolume);
function makeSalesVolumeGraph(salesVolume){

   // set the dimensions and margins of the graph
    var margin = {top: 30, right: 20, bottom: 160, left: 80},
        width = getWidthOfGraph('#salesVolumeGraph') - margin.left - margin.right -20,
    ...
    x.domain(salesVolume.map(function(fishType) { return getReadableName(fishType[0]); }));
    y.domain([0, d3.max(salesVolume.map(function(fishType){ return fishType[1] }))]);

    // append the svg object to the body of the page
        // append a 'group' element to 'svg'
        // moves the 'group' element to the top left margin
    var svg = d3.select("#salesVolumeGraph").append("svg")
    ...
    var bars = svg.selectAll(".categoryBar")
      ...
      .enter().append("rect")
        ...
        .attr("x", function(d) { return x(getReadableName(d[0])); })
      ...
      bars.on("mouseover", function(d) {
          ...
          .on("focus", function(d) {
              ...
              var svgRect = d3.select("#salesVolumeGraph").node().getBoundingClientRect();
            tooltip.transition()
                .duration(200)
                .style("opacity", 0.9);
            tooltip .html(getReadableName(d[0])+" "+d[1])
	...
	svg.append("text")
        ...
        .text("Sales Volume Per Type During 2018");
